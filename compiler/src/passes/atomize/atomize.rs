    ...
    match expr {
        ...
        RExpr::Prim { op, args, typ } => {
            ...
            extras
                .into_iter()
                .flatten()
                .rfold(AExpr::Prim { op, args, typ }, |bdy, (sym, bnd)| AExpr::Let {
                    sym,
                    typ: bnd.typ().clone(),
                    bnd: Box::new(bnd),
                    bdy: Box::new(bdy),
        ...
        RExpr::Apply { fun, args, typ } => {
            ...
            fun_expr
                ...
                .rfold(AExpr::Apply { fun, args, typ }, |bdy, (sym, bnd)| AExpr::Let {
                    typ: bnd.typ().clone(),
                    sym,
                    bnd: Box::new(bnd),
                    bdy: Box::new(bdy),
        ...
        RExpr::Struct { sym, fields, typ } => {
            ...
            extras
                .into_iter()
                .flatten()
                .rfold(AExpr::Struct { sym, fields, typ }, |bdy, (sym, bnd)| {
                    AExpr::Let {
                        sym,
                        typ: bnd.typ().clone(),
                        bnd: Box::new(bnd),
        ...
        RExpr::AccessField { strct, field, typ } => {
            ...
            extra
                .into_iter()
                .rfold(AExpr::AccessField { strct, field, typ }, |bdy, (sym, bnd)| {
                    AExpr::Let {
                        sym,
                        typ: bnd.typ().clone(),
                        bnd: Box::new(bnd),
