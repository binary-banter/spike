...
use crate::passes::validate::constrain::{
    access_field, apply, assign, binary_op, seq, unary_op, var,
};
...
pub fn constrain_expr<'p>(
    ...
    match expr.inner {
        ...
        Expr::Let {
            ...
            bnd,
            bdy,
        } => crate::passes::validate::constrain::r#let::constrain_let(
            env, span, sym, mutable, typ, bnd, bdy,
        ),
        Expr::If { cnd, thn, els } => {
            crate::passes::validate::constrain::r#if::constrain_if(env, span, cnd, thn, els)
        }
        Expr::Apply { fun, args } => apply::constrain_apply(env, span, fun, args),
        Expr::Loop { bdy } => {
            crate::passes::validate::constrain::r#loop::constrain_loop(env, span, bdy)
        }
        Expr::Break { bdy } => {
            crate::passes::validate::constrain::r#break::constrain_break(env, span, bdy)
        }
        Expr::Continue => {
            crate::passes::validate::constrain::r#continue::constrain_continue(env, span)
        }
        Expr::Return { bdy } => {
            crate::passes::validate::constrain::r#return::constrain_return(env, span, bdy)
        }
        ...
        Expr::Struct { sym, fields } => {
            crate::passes::validate::constrain::r#struct::constrain_struct(env, span, sym, fields)
        }
        Expr::AccessField { strct, field } => {
            access_field::constrain_access_field(env, span, strct, field)
        }
        Expr::Variant { .. } => todo!(),
        Expr::Switch { .. } => todo!(),
    }
