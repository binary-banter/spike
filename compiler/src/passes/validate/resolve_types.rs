...
use crate::passes::validate::{CMeta, DefConstrained, DefValidated, ExprConstrained, ExprValidated, PrgConstrained, PrgValidated, TLit};
...
impl<'p> PrgConstrained<'p> {
    pub fn resolve(mut self) -> Result<PrgValidated<'p>, TypeError> {
        Ok(PrgValidated {
            defs: self.defs.into_iter().map(|(sym, def)| resolve_def(def, &mut self.uf).map(|def| (sym, def))).collect::<Result<_, _>>()?,
            entry: self.entry,
        })
    }
}

fn resolve_def<'p>(def: DefConstrained<'p>, uf: &mut UnionFind<PartialType<'p>>) -> Result<DefValidated<'p>, TypeError>{
    let def = match def {
        DefConstrained::Fn { sym, params, typ, bdy } => {
            DefValidated::Fn {
                sym: sym.inner,
                params: params.fmap(|p| {
                    Param {
                        sym: p.sym.inner,
...
fn resolve_typedef<'p>(typedef: TypeDef<Meta<Span, UniqueSym<'p>>, Meta<Span, &'p str>>) -> TypeDef<UniqueSym<'p>, &'p str>{
    match typedef {
        TypeDef::Struct { fields } => TypeDef::Struct {
            fields: fields.fmap(|(field_sym, field_typ)| (field_sym.inner, resolve_type(field_typ)))
        },
        TypeDef::Enum { .. } => todo!()
    }
}
...
fn resolve_type(typ: Type<Meta<Span, UniqueSym>>) -> Type<UniqueSym> {
    match typ {
        ...
        Type::Fn { params, typ } => Type::Fn {params: params.fmap(resolve_type), typ: typ.fmap(resolve_type) },
        Type::Var { sym } => Type::Var { sym: sym.inner },
    }
}

fn resolve_expr<'p>(expr: Meta<CMeta, ExprConstrained<'p>>, uf: &mut UnionFind<PartialType<'p>>) -> Result<Meta<Type<UniqueSym<'p>>, ExprValidated<'p>>, TypeError>{
