...
impl IO for StdIO {
    fn read(&mut self) -> i64 {
        ...
        stdin().read_line(&mut input).expect("IO error or something");
...
impl IO for TestIO {
    fn read(&mut self) -> i64 {
        self.inputs.next().expect("Test tried to read more input than were available.")
    }
...
mod tests{
    ...
    fn interpret([test]: [&str; 1]) {
        ...
        let input = input.lines().filter(|l| !l.is_empty()).map(|l| l.parse().unwrap()).collect();
        let expected_output: Vec<_> = expected_output.lines().filter(|l| !l.is_empty()).map(|l| l.parse().unwrap()).collect();
