...
fn encode_push_pop(op_info: PushPopInfo, reg: &Arg) -> Vec<u8> {
    match reg {
        Arg::Imm { val } => {
            let val = *val as i32;
...
mod tests {
    ...
    mod push {
        ...
        check!(deref1, pushq!(deref!(RDX, i32::MAX as i64)), vec![0xFF, 0xB2, 0xFF, 0xFF, 0xFF, 0x7F]);
        check!(deref2, pushq!(deref!(R11, i32::MAX as i64)), vec![0x41, 0xFF, 0xB3, 0xFF, 0xFF, 0xFF, 0x7F ]);

        check!(imm, pushq!(imm!(i32::MAX as i64)), vec![ 0x68, 0xFF, 0xFF, 0xFF, 0x7F]);
    }
    ...
    mod pop {
        ...
        check!(deref1, popq!(deref!(RDX, i32::MAX as i64)), vec![0x8F, 0x82, 0xFF, 0xFF, 0xFF, 0x7F]);
        check!(deref2, popq!(deref!(R11, i32::MAX as i64)), vec![0x41, 0x8F, 0x83, 0xFF, 0xFF, 0xFF, 0x7F]);
    }
}
