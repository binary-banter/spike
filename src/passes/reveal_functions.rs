...
impl<'p> PrgUniquified<'p>{
    pub fn reveal(self) -> PrgRevealed<'p> {
        ...
        PrgRevealed{
            defs: self.defs.into_iter().map(|(sym, def)| {
                let def = match def {
                    Def::Fn { sym, prms, typ, bdy } => RDef::Fn {
                        sym,
                        prms,
                        typ,
...
fn reveal_expr<'p>(expr: Expr<UniqueSym<'p>>, scope: &mut PushMap<UniqueSym<'p>, ()>) -> RExpr<'p> {
    match expr {
        ...
        Expr::Var { sym } => if scope.contains(&sym) {
            RExpr::FunRef { sym }
        } else {
            RExpr::Var { sym }
        },
        Expr::Prim { op, args } => {
            RExpr::Prim { op, args: args.into_iter().map(|arg| reveal_expr(arg, scope)).collect() }
        },
        Expr::Let { sym, bnd, bdy } => {
            let bnd = Box::new(reveal_expr(*bnd, scope));
            scope.remove(sym, |scope| {
                RExpr::Let {
                    sym,
                    bnd,
        ...
        Expr::Apply { fun, args } => RExpr::Apply {
                ...
                args: args.into_iter().map(|arg| reveal_expr(arg, scope)).collect()
        },
    }
}
...
mod tests {
    ...
    fn reveal([test]: [&str; 1]) {
        ...
        let uniquified_program: PrgUniquified = program.type_check().unwrap().uniquify().reveal().into();
